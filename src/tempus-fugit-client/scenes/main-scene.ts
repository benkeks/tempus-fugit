...
export class MainScene extends Phaser.Scene implements GameStateListener {
  ...
  create(): void {
    ...
    storyDialog.triggerFunction = function (game:Game) {return game.getTurnCount() >= 0};
    let t2:string[][] = [["1", "you are stronger than expected!"]];
    let s2:StoryDialog = new StoryDialog(t2);
    s2.triggerFunction = function (game:Game) {return game.enemys[0].currentHP <=2};
  ...
  storyDialog(game: Game, dialog: StoryDialog): void {
